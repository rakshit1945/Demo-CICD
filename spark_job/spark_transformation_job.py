...
from pyspark.sql.functions import col, when, lit, expr, count, avg
import logging
import sys
import argparse
...
def main(env, bq_project, bq_dataset, transformed_table, route_insights_table, origin_insights_table):
    try:
        ...
        input_path = f"gs://airflow-projects-rj/source-{env}"
        logger.info(f"Input path has been resolved: {input_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", required=True, help="Defines the environment, e.g., dev or prod")
